 ...
 *
 */
class Tinkoff_Project_Helper {

    private $_release = "2.0.0";
    ...
    public function basic(){
        if (!isset($this->_basic)) $this->_basic = new Tinkoff_Basicmode();
        return $this->_basic;
    }

    public function result(){
        if (!isset($this->_result)) $this->_result = new Tinkoff_Result();
        return $this->_result;
    }

    public function postback(){
        if (!isset($this->_postback)) $this->_postback = new Tinkoff_Postback();
            ...
            if ($handle) {
                while (false !== ($file = readdir($handle))) {
                    if ($file != "." && $file != ".."){
                        ...
                        $className = "Tinkoff_Paymentmethod_".ucfirst($name);
...
/**
 *  Tinkoff_StatusCode static class
 *  Contains the payment statuscode constants
 *  @author Olaf Abbenhuis
 */
class Tinkoff_StatusCode {
    const OPEN           = "OPEN";
    const ERROR          = "ERR";
    const SUCCESS        = "OK";
    const REFUND         = "REFUND";
...
/**
 *  Tinkoff_Basicmode_Interface_Abstract interface
 *  @author Olaf Abbenhuis
 */
interface Tinkoff_Basicmode_Interface_Abstract {
...
/**
 *  Tinkoff_Basicmode class
 *  To start a basicmode payment
 *  @author Olaf Abbenhuis
 */
class Tinkoff_Basicmode implements Tinkoff_Basicmode_Interface_Abstract {

    protected $_basicmodeURL = "pay.tinkoff.eu/basic/";
    ...
    */
    ...
    /**
    * Post the fields and return the URL generated by TINKOFF
    * @since version 1.0.0
    * @access public
    ...
    */
    ...
    /**
    * Generates a URL to the TINKOFF basic API service
    * @since version 1.0.0
    * @access protected
    * @return string URL
    */
    ...
    /**
    * Used to connect to the TINKOFF servers
    * @since version 1.0.0
    * @access protected
    * @param string $url
    * @param array $data
    ...
    * @since version 1.0.0
    * @access public
    * @param string $issuer TINKOFF Issuer code
    */
    public function setIssuer($issuer){
...
/**
 *  Tinkoff_Postback class
 *  To handle the postback
 *  @author Olaf Abbenhuis
 */
class Tinkoff_Postback extends Tinkoff_Basicmode {
    ...
    */
    public function validate() {
        ...
        if (!in_array(strtoupper($this->data->status), array(
                    Tinkoff_StatusCode::OPEN,
                    Tinkoff_StatusCode::SUCCESS,
                    Tinkoff_StatusCode::ERROR,
                    Tinkoff_StatusCode::REFUND,
                    Tinkoff_StatusCode::CHARGEBACK
                ))) {
    ...
    */
    ...
    /**
    * Check between TINKOFF statuscodes whether the status can be updated.
    * @since version 1.0.0
    * @access public
    * @param string $currentStatus The TINKOFF statuscode of the order before a statuschange
    * @return boolean
    */
    public function canUpdateStatus($currentStatus) {
        ...
        switch ($this->data->status) {
            case Tinkoff_StatusCode::SUCCESS:       return ($currentStatus == Tinkoff_StatusCode::OPEN);
            case Tinkoff_StatusCode::OPEN:          return ($currentStatus == Tinkoff_StatusCode::OPEN);
            case Tinkoff_StatusCode::ERROR:         return ($currentStatus == Tinkoff_StatusCode::OPEN);
            case Tinkoff_StatusCode::CHARGEBACK:    return ($currentStatus == Tinkoff_StatusCode::SUCCESS);
            case Tinkoff_StatusCode::REFUND:        return ($currentStatus == Tinkoff_StatusCode::SUCCESS);
            default:
                return false;
        };
    }
...
/**
 *  Tinkoff_Result class
 *  To handle the success and error page
 *  @author Olaf Abbenhuis
 */
class Tinkoff_Result extends Tinkoff_Basicmode {

    /**
    * Validate the TINKOFF GET data
    * @since version 1.0.0
    * @access public
    * @return boolean
    */
