...
/******/ 	return __webpack_require__(__webpack_require__.s = "./src/pages/steps/index.js");
/******/ })
/************************************************************************/
/******/ ({
...
/*!*********************************************************!*\
  !*** ./node_modules/lit-html/directives/unsafe-html.js ***!
  \*********************************************************/
/*! exports provided: UnsafeHTMLDirective, unsafeHTML */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "UnsafeHTMLDirective", function() { return e; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "unsafeHTML", function() { return o; });
...
"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony import */ var _scripts__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./scripts */ "./src/pages/steps/scripts.js");
/* harmony import */ var _style_scss__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./style.scss */ "./src/pages/steps/style.scss");
...
var ELEM_NEXT_STEP = '.nextStep';
var ELEM_PREV_STEP = '.prevStep';

			if (typeof UIkit === 'undefined') {
				var script = document.createElement('script');
				script.src = '/widget/shop/dist/uikit.min.js';
...
var Shop = function Shop(localData) {
  var _this = this;
  ...
  _defineProperty(this, "availibleSales", {
    active: [],
    total: 0,
	total_install: 0
  });
  ...
  _defineProperty(this, "changeShop", new Event('shop:changeShop'));

  _defineProperty(this, "STEPS", new Steps());

  _defineProperty(this, "DELEVERY", new Delivery());

  _defineProperty(this, "PAYMENT", new Payment());

  _defineProperty(this, "initProducts", function (data) {
    _this.products = data.products;
    _this.sales = data.sales;

    _this.drawProducts();

    _this.recalculateCart();

    _this.drawTotal();

    _this.init = true;
    document.dispatchEvent(_this.changeShop);
  });

  _defineProperty(this, "changeCartEvent", function () {
    localStorage.setItem('cart', JSON.stringify(_this.cart));

    _this.drawTotal();

    if (_this.STEPS.step === 3) {
		let pcnt = 0;
		$.each(_this.cart, function(key, value) {
			pcnt += value.count;
		});

      $('#productCount').text("".concat(pcnt, " ").concat(num_word(pcnt, ['товар', 'товара', 'товаров'])));
      $('#productCost').text(_this.totalPrice);
    }

	  if (!$('#products_attr').is(':visible')) {
		  $('#step1_btn, #step1_btn_back').hide();
		  $('#step1_attr_btn').show();
	  } else {
		  $('#step1_btn, #step1_btn_back').show();
		  $('#step1_attr_btn').hide();
	  }
	...
	$(document).on('click', '.gotoattr', function () {
		$('#step1_attr_btn').hide();
		$('#step1_btn, #step1_btn_back').show();
		$('#products').hide();
		$('#products_attr').css('display', 'flex');
	});

	$('#step1_btn_back').click(function(){
		$('#step1_attr_btn').show();
		$('#step1_btn, #step1_btn_back').hide();
		$('#products').show();
		$('#products_attr').hide();
		...
		$('html, body').animate({
			scrollTop: $('#step-1').offset().top - top_p
		}, 1000);
	});

	$('#step1_attr_btn').click(function(){
		if (!$('#products .el-content.remove').length) {
			UIkit.modal('<div uk-modal class="uk-modal uk-flex-top uk-flex uk-open" id="modal-0"><div class="uk-modal-dialog uk-modal-body uk-margin-auto-vertical"><p>Сначала добавьте основной товар к заказу</p><p class="uk-text-right"><button type="button" class="uk-button uk-button-default uk-modal-close btnCart add" data-offerid="' + $('#products a[data-product]').data('offerid') + '" data-product="' + $('#products a[data-product]').data('product') + '">Добавить</button> <span class="uk-modal-close gotoattr uk-text-warning" type="button">Выбрать аксессуары</span></p></div></div>').show();
		} else {
			$(this).hide();
			$('#products').hide();
			$('#step1_btn, #step1_btn_back').show();
  ...
  _defineProperty(this, "drawProducts", function () {
    var productsTemplates = function productsTemplates(products) {
      return Object(lit_html__WEBPACK_IMPORTED_MODULE_1__["html"])(_templateObject || (_templateObject = _taggedTemplateLiteral(["", ""])), products.map(function (product) {
        var options = {
          inCart: false,
          count: 1
        };

        if (_this.existInCart(product.id)) {
          options = _objectSpread(_objectSpread({}, options), {}, {
            inCart: true,
            count: _this.cart.find(function (item) {
              return +item.id === +product.id;
            }).count
          });
        }

        return Object(_templates__WEBPACK_IMPORTED_MODULE_3__["productTemplate"])(product, options);
      }));
    };

    Object(lit_html__WEBPACK_IMPORTED_MODULE_1__["render"])(productsTemplates(_this.products), $('#products')[0]);

    let attr_cnt = 0;
	$.each(_this.products, function( key, item) {
		if (item.attr.length) {
			attr_cnt += item.attr.length;
			Object(lit_html__WEBPACK_IMPORTED_MODULE_1__["render"])(productsTemplates(item.attr), $('#products_attr')[0]);
		}
	});

	if (attr_cnt <= 4) {
		$('#products_attr').removeClass('uk-child-width-1-3@m').addClass('uk-child-width-1-4@m');
	}
  });

  _defineProperty(this, "existInCart", function (id) {
    return _this.cart.filter(function (item) {
      return +item.id === +id;
    }).length > 0 ? true : false;
  });

  _defineProperty(this, "recalculateCart", function () {
    var total = 0;
    var old_total = _this.totalPrice;

    _this.cart.forEach(function (cartItem) {
      _this.products.forEach(function (product) {
        if (+cartItem.id === +product.id) {
          total += cartItem.count * parseFloat(product.price);
        }

		if (product.attr.length) {
			product.attr.forEach(function (product2) {
				if (+cartItem.id === +product2.id) {
					total += cartItem.count * parseFloat(product2.price);
				}
			});
		}
      });
    ...
    _this.recalculateSales(total, old_total);

    _this.totalPrice = total;

    _this.changeCartEvent();
  });

  _defineProperty(this, "recalculateSales", function () {
    var total = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : _this.totalPrice;
    var old_total = arguments[1];

	var ids_arr = [];
	$.each(_this.cart, function(index, item) {
		ids_arr.push(item.id);
	});

	ids_arr.sort(function(a, b) {
		return a - b;
	});

	var sale_cost_pr = 0;
	var total_install = 0;
	var sale_cost_total = 0;
	//var sale_str = 'prod_' + ids_arr.join('_');

	if ($('#products .el-content > .btnCart.remove').length && $('#products_attr .el-content > .btnCart.remove').length >= 3) {
		var sale_str = 'prod_4';

		var sale_id = _this.sales.find(function (item) {
			return item.code === sale_str;
		});

		if (sale_id) {
			var sale_cost = sale_id.cost;
			if (sale_cost.toString().indexOf('%') !== -1) {
				sale_cost = total * (sale_cost.slice(0, -1)) / 100;
			}

			sale_cost = Math.round(parseFloat(sale_cost));
			sale_cost_pr = sale_cost;
			sale_cost_total += sale_cost;
		}
	}

	if (_this.PAYMENT.selected && _this.PAYMENT.selected.code === 'installment' && (_this.STEPS.step == 2 || _this.STEPS.step == 3)) {
		var sale_ins = _this.sales.find(function (item) {
			return item.code === 'installment';
		});

		var new_total = total + _this.DELEVERY.cost;
		if (sale_cost_pr) {
			new_total = total - sale_cost_pr;
		}

		new_total += _this.DELEVERY.cost;

		var sale_cost = sale_ins.cost;
		if (sale_cost.toString().indexOf('%') !== -1) {
			sale_cost = new_total * (sale_cost.slice(0, -1)) / 100;
		}

		total_install = Math.round(parseFloat(sale_cost));
	}

	  if (_this.PAYMENT.selected && _this.PAYMENT.selected.code === 'bank-sbp' && (_this.STEPS.step == 2 || _this.STEPS.step == 3)) {
		  var sale_ins = _this.sales.find(function (item) {
			  return item.code === 'bank-sbp';
		  });
	  ...
	  _this.availibleSales = _objectSpread(_objectSpread({}, _this.availibleSales), {}, {
		  active: [],
		  total: sale_cost_total,
		  total_install: total_install
	  });

    // sale cart_20000
    /*if (total >= 20000) {
      var sale = _this.sales.find(function (item) {
        return item.code === 'cart_20000';
      });

      if (_this.availibleSales.active.filter(function (code) {
        return code === 'cart_20000';
      }).length === 0) {
        _this.availibleSales = _objectSpread(_objectSpread({}, _this.availibleSales), {}, {
          active: [].concat(_toConsumableArray(_this.availibleSales.active), [sale.code]),
          total: _this.availibleSales.total + sale.cost
        });
      }
    } else if (total < 20000) {
      var _sale = _this.sales.find(function (item) {
        return item.code === 'cart_20000';
      });
      ...
      if (_this.availibleSales.active.filter(function (code) {
        ...
        _this.availibleSales = _objectSpread(_objectSpread({}, _this.availibleSales), {}, {
          active: _this.availibleSales.active.filter(function (code) {
            return code !== 'cart_20000';
          }),
          total: _this.availibleSales.total - _sale.cost
        });
      }
    }*/ // sale prepayment_card_100

    /*if (_this.PAYMENT.selected && _this.PAYMENT.selected.code === 'bank-transfer') {
      var _sale2 = _this.sales.find(function (item) {
        return item.code === 'prepayment_card_100';
      });

      if (_this.availibleSales.active.filter(function (code) {
        return code === 'prepayment_card_100';
      }).length === 0) {
        _this.availibleSales = _objectSpread(_objectSpread({}, _this.availibleSales), {}, {
          active: [].concat(_toConsumableArray(_this.availibleSales.active), [_sale2.code]),
          total: _this.availibleSales.total + _sale2.cost
        });
      }
    } else {
      if (_this.availibleSales.active.filter(function (code) {
        ...
        var _sale3 = _this.sales.find(function (item) {
          return item.code === 'prepayment_card_100';
        });
        ...
        _this.availibleSales = _objectSpread(_objectSpread({}, _this.availibleSales), {}, {
          ...
          total: _this.availibleSales.total - _sale3.cost
        });
      }
    } // sale payment_card_online


    if (_this.PAYMENT.selected && _this.PAYMENT.selected.code === 'payment_card_online') {
      var _sale4 = _this.sales.find(function (item) {
        return item.code === 'payment_card_online';
      });

      if (_this.availibleSales.active.filter(function (code) {
        return code === 'payment_card_online';
      }).length === 0) {
        _this.availibleSales = _objectSpread(_objectSpread({}, _this.availibleSales), {}, {
          active: [].concat(_toConsumableArray(_this.availibleSales.active), [_sale4.code]),
          total: _this.availibleSales.total + _sale4.cost
        });
      }
    } else {
      if (_this.availibleSales.active.filter(function (code) {
        ...
        var _sale5 = _this.sales.find(function (item) {
          return item.code === 'payment_card_online';
        });
        ...
        _this.availibleSales = _objectSpread(_objectSpread({}, _this.availibleSales), {}, {
          ...
          total: _this.availibleSales.total - _sale5.cost
        });
      }
    }*/
  ...
  _defineProperty(this, "addProduct", function (id, count, offerID) {
    if (_this.existInCart(id)) {
      _this.cart = _this.cart.map(function (item) {
        if (+item.id === +id) {
      ...
      _this.cart = [].concat(_toConsumableArray(_this.cart), [{
        id: id,
        count: count,
        offerID: offerID
      }]);
    ...
    $.getJSON('/widget/api/config.json', function (conf) {
    	let metrika = conf.metrika;

    	$.each(metrika.addtocart, function (goal, ids) {
			let addgoal = getCookie('addgoal' + goal);

			if (!addgoal) {
				let ids_arr = ids.split(',');

				$.each(ids_arr, function (key, pid) {
					if (id == pid) {
						ym(62782636, 'reachGoal', goal);
						setCookie('addgoal' + goal, 1);
					}
				});
			}
    	});
  ...
  _defineProperty(this, "removeProduct", function (id) {
    _this.cart = _this.cart.filter(function (item) {
      return +item.id !== +id;
    });
  ...
  _defineProperty(this, "changeCount", function (id, count) {
    _this.cart = _this.cart.map(function (item) {
      if (+id === +item.id) {
        return _objectSpread(_objectSpread({}, item), {}, {
          count: count
  ...
  _defineProperty(this, "drawTotal", function () {
  	let pcnt = 0;
	  $.each(_this.cart, function(key, value) {
		  pcnt += value.count;
	  });

    Object(lit_html__WEBPACK_IMPORTED_MODULE_1__["render"])(Object(_templates__WEBPACK_IMPORTED_MODULE_3__["productTotalTemplate"])(pcnt, _this.totalPrice, _this.availibleSales.total), $('#totalProducts')[0]);
    $('#totalProducts').show();
    $('#order_sales_total').text($('#sales_total').text());
   // $('#order_margin').text(_this.availibleSales.total_install);
  });

  _defineProperty(this, "drawOrder", function () {
    var typeDeliveryName = '';
    var providerName = '';

    switch (_this.DELEVERY.service) {
      case 'toPoint':
    ...
    switch (_this.DELEVERY.provider) {
      case 'cdek':
        providerName = 'CDEK';
        break;
	  ...
	  $('#input-phone').inputmask(phone_code + ' (999) 999-9999');

    _this.contacts = localStorage.getItem('contacts');
    ...
    $('#productCount').text("".concat(pcnt, " ").concat(num_word(pcnt, ['товар', 'товара', 'товаров'])));
    $('#productCost').text(_this.totalPrice);
    $('#deliveryCity').text(_this.DELEVERY.city.value);
    $('#deliveryProvider').text(providerName);
    $('#deliveryService').text(typeDeliveryName);
    setTimeout(function(){
    	$('.salesTotal').text(_this.availibleSales.total);
		//$('.marginTotal').text(_this.availibleSales.total_install);
    }, 500);

    if (_this.DELEVERY.service === 'toPoint') {
      $('#deliveryAddress').text(_this.DELEVERY.address.address);
    } else {
      $('#deliveryAddress').text(_this.DELEVERY.address.value);
    }

    if (_this.DELEVERY.deliveryType && _this.DELEVERY.deliveryType.daysMax !== undefined) {
		$('#deliveryDays').text("".concat(_this.DELEVERY.deliveryType.daysMax, " ").concat(num_word(_this.DELEVERY.deliveryType.daysMax, ['день', 'дня', 'дней'])));
	}

    if (_this.DELEVERY.deliveryType && _this.DELEVERY.deliveryType.deliveryCost !== undefined) {
    	$('#deliveryCost').text(_this.DELEVERY.deliveryType.deliveryCost);
    }

    if (_this.PAYMENT.selected !== null) {
		$('#paymentType').text(_this.PAYMENT.selected.name);
	}
  });
...
var Steps = function Steps() {
  ...
  _defineProperty(this, "countSteps", 3);

  _defineProperty(this, "changeStepEvent", new Event('shop:changeStep'));

  _defineProperty(this, "setStep", function (step) {
    if (step >= 1 && step <= 3) {
      $('.step').hide();
      $("#step-".concat(step)).show();
      localStorage.setItem('step', step);
      document.dispatchEvent(_this2.changeStepEvent);
    }
	  ...
	  if (step == 3) {
		  ...
		  if( /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent) ) {
			  top_p = 0;
				  ...
				  $('html, body').animate({
					  scrollTop: $('#selectMapPoint').offset().top - top_p
				  }, 1000);
  ...
  var localStep = +localStorage.getItem('step');
...
var Delivery = function Delivery() {
  ...
  _defineProperty(this, "service", null);

  _defineProperty(this, "address", null);

  _defineProperty(this, "cost", 0);

  _defineProperty(this, "deliveryType", null);

  _defineProperty(this, "changeDelivery", new Event('shop:changeDelivery'));

  _defineProperty(this, "availible", false);

  _defineProperty(this, "countries", []);

  _defineProperty(this, "cities", []);

  _defineProperty(this, "pvz", []);

  _defineProperty(this, "addresses", []);

  _defineProperty(this, "deliveryTypes", []);

  _defineProperty(this, "changeDeliveryEvent", function () {
    localStorage.setItem('delivery', JSON.stringify({
      ...
      cost: _this3.cost,
      deliveryType: _this3.deliveryType
    }));
    document.dispatchEvent(_this3.changeDelivery);

    if (_this3.address === null) {
      $('#totalDelivery').hide();
    }
  });

  _defineProperty(this, "initDelivery", function () {
    if (_this3.country === null) {
      $('#selectCity, #selectTypeDelivery, #selectMapPoint, #selectCurier, #totalDelivery').hide();
		$('.lds-wrapper, .lds-ring').show();
      jquery__WEBPACK_IMPORTED_MODULE_0___default.a.get('/widget/api/address.php?type=country&query=Россия', function (data) {
        if (data && data.length) {
          _this3.country = data[0];
          $('#country').val(_this3.country.value);
          $("#selectCity").show();
			$('.lds-wrapper, .lds-ring').hide();
          _this3.changeDeliveryEvent();
        }
      });
    } else if (_this3.city === null) {
      $('#country').val(_this3.country.value);
      $('#selectTypeDelivery, #selectMapPoint, #selectCurier, #totalDelivery').hide();
    } else {
      $('#country').val(_this3.country.value);
      $('#city').val(_this3.city.value);
      $('#selectMapPoint, #selectCurier, #totalDelivery').hide();

      _this3.drawDeliveries();

      if (_this3.address) {
        _this3.drawTotalDelivery();
      } else {
        $('#totalDelivery').hide();
      }
    }

		/*if (_this3.country === null || _this3.city === null || _this3.address === null || _this3.deliveryType === null || !$('input[name="payment"]').is(':checked')) {
		  $(ELEM_NEXT_STEP).hide();alert($('input[name="payment"]').is(':checked'));
		}*/
  ...
  _defineProperty(this, "searchCountry", function (query) {
    //if (query.length >= 3) {
		$('.lds-wrapper, .lds-ring').show();
      jquery__WEBPACK_IMPORTED_MODULE_0___default.a.get("/widget/api/address.php?type=country&query=".concat(query), function (data) {
        _this3.countries = data;
		  ...
		  if (citiesListtimer) {
			  clearTimeout(citiesListtimer);
		  }
		  _this3.country = _this3.countries[0];
		  $('#country').val(_this3.country.value);
		  _this3.city = null;
		  _this3.service = null;
		  _this3.provider = null;
		  _this3.deliveryType = null;
		  $("#city").val('');
		  $("#selectTypeDelivery, #selectMapPoint, #selectCurier, #totalDelivery").hide();
		  $("#selectCity").show();
		  ...
		  _this3.changeDeliveryEvent();
		  $('.lds-wrapper, .lds-ring').hide();
        /*var listItems = function listItems(result) {
          return Object(lit_html__WEBPACK_IMPORTED_MODULE_1__["html"])(_templateObject2 || (_templateObject2 = _taggedTemplateLiteral(["", ""])), result.map(function (item, i) {
            return Object(lit_html__WEBPACK_IMPORTED_MODULE_1__["html"])(_templateObject3 || (_templateObject3 = _taggedTemplateLiteral(["\n                        <li><button type=\"button\" data-index=\"", "\" data-value=\"", "\">", "</button></li>\n                    "])), i, item.value, item.value);
          }));
        };*/

        //Object(lit_html__WEBPACK_IMPORTED_MODULE_1__["render"])(listItems(data), $('#contriesList')[0]);
        //$('#contriesList').show();
      });
    //}
  });

  _defineProperty(this, "selectCountry", function (index) {
    _this3.country = _this3.countries[0];
    $('#country').val(_this3.country.value);
    _this3.city = null;
    _this3.service = null;
  ...
  _defineProperty(this, "searchCity", function (query) {
    if (query.length >= 3) {
      if (_this3.country.data.alfa2 !== 'RU') {
		  $('.lds-wrapper, .lds-ring').show();
        jquery__WEBPACK_IMPORTED_MODULE_0___default.a.get("/widget/api/address.php?type=city&country=".concat(_this3.country.data.alfa2, "&query=").concat(query), function (data) {
          _this3.cities = data;
			$('.lds-wrapper, .lds-ring').hide();
          var listItems = function listItems(result) {
            return Object(lit_html__WEBPACK_IMPORTED_MODULE_1__["html"])(_templateObject4 || (_templateObject4 = _taggedTemplateLiteral(["", ""])), result.map(function (item, i) {
              return Object(lit_html__WEBPACK_IMPORTED_MODULE_1__["html"])(_templateObject5 || (_templateObject5 = _taggedTemplateLiteral(["\n                            <li><button type=\"button\" data-index=\"", "\" data-value=\"", "\">", "</button></li>\n                        "])), i, item.value, item.value);
            }));
          };

          Object(lit_html__WEBPACK_IMPORTED_MODULE_1__["render"])(listItems(data), $('#citiesList')[0]);
          $('#citiesList').show();
        });
      } else {
		  $('.lds-wrapper, .lds-ring').show();
        jquery__WEBPACK_IMPORTED_MODULE_0___default.a.get("/widget/api/address.php?type=city&query=".concat(query), function (data) {
          _this3.cities = data;
			$('.lds-wrapper, .lds-ring').hide();
          var listItems = function listItems(result) {
            return Object(lit_html__WEBPACK_IMPORTED_MODULE_1__["html"])(_templateObject6 || (_templateObject6 = _taggedTemplateLiteral(["", ""])), result.map(function (item, i) {
              return Object(lit_html__WEBPACK_IMPORTED_MODULE_1__["html"])(_templateObject7 || (_templateObject7 = _taggedTemplateLiteral(["\n                            <li><button type=\"button\" data-index=\"", "\" data-value=\"", "\">", "</button></li>\n                        "])), i, item.value, item.value);
            }));
          };
  ...
  _defineProperty(this, "selectCity", function (index) {
    _this3.city = _this3.cities[index];
    $('#city').val(_this3.city.value);
    //$('#city').val(_this3.city.data.city_with_type);
    _this3.service = null;
    _this3.provider = null;
    _this3.deliveryType = null;
    ...
    if (_this3.country.data.alfa2 !== 'RU') {
		$('.lds-wrapper, .lds-ring').show();
      jquery__WEBPACK_IMPORTED_MODULE_0___default.a.get("/widget/api/address.php?type=city_geo&city=".concat(_this3.city.data.name, "&country=").concat(_this3.country.data.alfa2), function (response) {
		  $('.lds-wrapper, .lds-ring').hide();
        _this3.city.data.geo_lat = response.lat;
        _this3.city.data.geo_lon = response.lon;
  ...
  _defineProperty(this, "drawDeliveries", function () {
    var city = _this3.country.data.alfa2 === 'RU' ? _this3.city.data.fias_id : _this3.city.data.name;
    var cityType = _this3.country.data.alfa2 === 'RU' ? 'fias' : 'name';
	  $('.lds-wrapper, .lds-ring').show();
	  ...
	  var cnts = [];
	  var ids = [];
	  $.each(localShopData.cart, function(index, value) {
		  ids.push(value.id);
		  cnts.push(value.id + '|' + value.count);
	  });

    jquery__WEBPACK_IMPORTED_MODULE_0___default.a.get("/widget/api/delivery.php?cnts=" + cnts.join(',') + "&ids=" + ids.join(',') +"&type=cost&cityType=".concat(cityType, "&city=").concat(city, "&country=").concat(_this3.country.data.alfa2), function (data) {
      _this3.deliveryTypes = data;
		$('.lds-wrapper, .lds-ring').hide();
      var deliveryTemplates = function deliveryTemplates(deliveries) {
        var availible = 0;
        ...
        if (availible) {
          _this3.availible = true;

          if (_this3.service === 'toPoint') {
            _this3.drawMapPvz();

            $('#selectMapPoint').show();
            $('#selectCurier').hide();
          } else if (_this3.service === 'toDoor') {
            if (_this3.address) {
              $('#addressCurier').val(_this3.address.value);

				if (_this3.address.data.house) {
					$('#houseCurier').val(_this3.address.data.house);
				}
            }
          ...
          return Object(lit_html__WEBPACK_IMPORTED_MODULE_1__["html"])(_templateObject8 || (_templateObject8 = _taggedTemplateLiteral(["", ""])), deliveries.map(function (delivery, index) {
            var image = '';
            var name = '';

            switch (delivery.providerKey) {
              case 'dpd':
                image = '/widget/src/img/dpd.png';
                name = 'DPD - Курьерская служба';
                break;

              case 'cdek':
                image = '/widget/src/img/cdek.png';
                name = 'СДЭК - Курьерская служба';
                break;

              case 'rupost':
                image = '/widget/src/img/pochta.png';
                name = 'EMS Почта России';
                break;
				case 'ozon':
					image = '/widget/src/img/ozon.png';
					name = 'Ozon - Курьерская служба';
					break;
            ...
            return Object(_templates__WEBPACK_IMPORTED_MODULE_3__["deliveryTemplate"])(_objectSpread(_objectSpread({}, delivery), {}, {
              name: name,
              image: image,
              index: index
            }), _this3.provider, _this3.service);
          }));
        } else {
          _this3.availible = false;
          return 'Доставка для данного региона недоступна';
        }
      };

      Object(lit_html__WEBPACK_IMPORTED_MODULE_1__["render"])(deliveryTemplates(data), $('#selectTypeDelivery > .uk-grid')[0]);
      $('#selectTypeDelivery').show();
    });
  });

  _defineProperty(this, "selectDeliveryType", function (provider, service, cost, index) {
    ...
    if (service === 'toDoor') {
      _this3.deliveryType = _this3.deliveryTypes[index]['deliveryToDoor'];
    } else if (service === 'toPoint') {
      _this3.deliveryType = _this3.deliveryTypes[index]['deliveryToPoint'];
    }
    ...
    if (service === 'toPoint') {
      ...
      $('#selectCurier').show();
		...
		if( /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent) ) {
			...
			$('html, body').animate({
				scrollTop: $('#selectCurier').offset().top - top_p
			}, 1000);
    }
  ...
  _defineProperty(this, "drawMapPvz", function () {
    var map = $('#mapPvz');

    if (map.length && _this3.availible) {
      map.html(null);
      var center = [_this3.city.data.geo_lat, _this3.city.data.geo_lon];
      var city = _this3.country.data.alfa2 === 'RU' ? _this3.city.data.city_fias_id : _this3.city.data.name;
      var cityType = _this3.country.data.alfa2 === 'RU' ? 'fias' : 'name';
		$('.lds-wrapper, .lds-ring').show();
      jquery__WEBPACK_IMPORTED_MODULE_0___default.a.get("/widget/api/delivery.php?type=pvz&ids=" + _this3.deliveryType.pointIds.join(',') + "&cityType=".concat(cityType, "&city=").concat(city, "&country=").concat(_this3.country.data.alfa2, "&provider=").concat(_this3.provider), function (points) {
        _this3.pvz = points.rows;

        ymaps__WEBPACK_IMPORTED_MODULE_2__["default"].load('https://api-maps.yandex.ru/2.1/?lang=ru_RU&apikey=1fdd2c12-9957-40f9-a9d3-8d0e6311a00f').then(function (maps) {
          var myMap = new maps.Map('mapPvz', {
            center: center,
            zoom: 9
          });
          ...
          points.rows.forEach(function (pvz, i) {
            objectsPoints.features.push({
              ...
              properties: {
                balloonContent: "\n<button type=\"button\" data-id=\"" + pvz.id + "\" class=\"selectPvz\">\u0412\u044B\u0431\u0440\u0430\u0442\u044C</button> <br><b>\u0410\u0434\u0440\u0435\u0441:</b> ".concat(pvz.address, "<br>\n                                    <b>\u041A\u0430\u043A \u0434\u043E\u0431\u0440\u0430\u0442\u044C\u0441\u044F:</b> ").concat(pvz.description, "<br>\n                                    <b>\u0420\u0435\u0436\u0438\u043C \u0440\u0430\u0431\u043E\u0442\u044B:</b> ").concat(pvz.timetable, "<br>\n                                                                  "),
  ...
  _defineProperty(this, "selectPvz", function (id) {
    _this3.address = _this3.pvz.find(function (item) {
      return +item.id === id;
    });
  ...
  _defineProperty(this, "searchAddress", function (query) {
  	if (1) {
      if (query.length >= 3) {
		  var city = _this3.city.data.fias_id;
		  if (_this3.country.data.alfa2 != 'RU') {
			  city = _this3.city.data.name;
		  }

		  $('.lds-wrapper, .lds-ring').show();
        jquery__WEBPACK_IMPORTED_MODULE_0___default.a.get("/widget/api/address.php?c=" + $('#country').val() + "&b=" + city + "&query=".concat(query), function (data) {
          _this3.addresses = data;
			$('.lds-wrapper, .lds-ring').hide();
          var listItems = function listItems(result) {
            return Object(lit_html__WEBPACK_IMPORTED_MODULE_1__["html"])(_templateObject9 || (_templateObject9 = _taggedTemplateLiteral(["", ""])), result.map(function (item, i) {
              return Object(lit_html__WEBPACK_IMPORTED_MODULE_1__["html"])(_templateObject10 || (_templateObject10 = _taggedTemplateLiteral(["\n                            <li><button type=\"button\" data-index=\"", "\" data-value=\"", "\">", "</button></li>\n                        "])), i, item.value.replace(item.data.city_with_type + ', ', '').replace(item.data.region_with_type + ', ', ''), item.value.replace(item.data.city_with_type + ', ', '').replace(item.data.region_with_type + ', ', ''));
            }));
          };

          Object(lit_html__WEBPACK_IMPORTED_MODULE_1__["render"])(listItems(data), $('#addressList')[0]);
          $('#addressList').show();
        });
      }
    } else {
      ...
      var listItems = function listItems(result) {
        return Object(lit_html__WEBPACK_IMPORTED_MODULE_1__["html"])(_templateObject11 || (_templateObject11 = _taggedTemplateLiteral(["", ""])), result.map(function (item, i) {
          return Object(lit_html__WEBPACK_IMPORTED_MODULE_1__["html"])(_templateObject12 || (_templateObject12 = _taggedTemplateLiteral(["\n                    <li><button type=\"button\" data-index=\"", "\" data-value=\"", "\">", "</button></li>\n                "])), i, item.value, item.value);
        }));
      };
  ...
  _defineProperty(this, "selectAddress", function (index) {

    _this3.address = _this3.addresses[index];
    _this3.address.value = _this3.address.value.replace(_this3.address.data.city_with_type + ', ', '').replace(_this3.address.data.region_with_type + ', ', '');
   //$('#addressCurier').val(_this3.address.data.street_with_type);
    $('#addressCurier').val(_this3.address.value);

    if (_this3.address.data.house) {
		$('#houseCurier').val(_this3.address.data.house);
		//$('#addressCurier').removeClass('errorInp');
	} else {
		$('#houseCurier').val('');
		//$('#addressCurier').addClass('errorInp');
		//UIkit.modal.alert('Введите улицу, дом, квартиру');
	}
  ...
  _defineProperty(this, "drawTotalDelivery", function () {
    if (_this3.address === null) return;
    var dataDelivery = {
      service: null,
      provider: null,
      cost: _this3.cost,
    ...
    switch (_this3.provider) {
      ...
      case 'rupost':
        ...
        break;
		case 'ozon':
			dataDelivery.provider = 'Ozon';
			break;
    }

    switch (_this3.service) {
      case 'toPoint':
        dataDelivery.service = 'Адрес пункта самовывоза';
        dataDelivery.address = _this3.address.address;
        break;

      case 'toDoor':
        dataDelivery.service = 'Адрес доставки';
        dataDelivery.address = _this3.address.value;
        break;
    }

    Object(lit_html__WEBPACK_IMPORTED_MODULE_1__["render"])(Object(_templates__WEBPACK_IMPORTED_MODULE_3__["totalDeliveryTemplate"])(dataDelivery), $('#totalDelivery-1')[0]);
    $('#totalDelivery').show();
	  $('#order_sales_total').text($('#sales_total').text());
  });

  var localData = localStorage.getItem('delivery') ? JSON.parse(localStorage.getItem('delivery')) : null;
...
var Payment = function Payment() {
  ...
  _defineProperty(this, "payments", []);

  _defineProperty(this, "changePaymentEvent", function () {
  ...
  _defineProperty(this, "initPayments", function (country, service, address) {
	  $('.lds-wrapper, .lds-ring').show();
	  var localShopData = {
		  cart: localStorage.getItem('cart') ? JSON.parse(localStorage.getItem('cart')) : null,
		  delivery: localStorage.getItem('delivery') ? JSON.parse(localStorage.getItem('delivery')) : null
	  };
	  ...
	  jquery__WEBPACK_IMPORTED_MODULE_0___default.a.get('/widget/api/payments.php?cnts=' + cnts.join(',') + '&ids=' + ids.join(',') + '&type=get&country=' + localShopData.delivery.country.data.alfa2, function (response) {
		$('.lds-wrapper, .lds-ring').hide();
    	if (!response.error) {
        _this4.payments = response.data;

        _this4.drawPayments(country, service, address);
  ...
  _defineProperty(this, "drawPayments", function (country, service, address) {
    var availiblePayments = _toConsumableArray(_this4.payments);

    if (country === 'RU') {
      availiblePayments = availiblePayments.filter(function (item) {
        return item.code !== 'e-money';
      });
    }

    if (country !== 'RU') {
      availiblePayments = availiblePayments.filter(function (item) {
        return item.code !== 'credit';
      });
      availiblePayments = availiblePayments.filter(function (item) {
        return item.code !== 'bank-transfer';
      });
    }

    if (service === 'toPoint' && address.cod !== 1) {
      availiblePayments = availiblePayments.filter(function (item) {
        return item.code !== 'imposed';
      });
      ...
      if (infoPayment.length) {
        availiblePayments = availiblePayments.map(function (item) {
          if (item.code === 'imposed') {
            return _objectSpread(_objectSpread({}, item), {}, {
              name: "".concat(item.name).concat(infoPayment)
            });
          }
    ...
    var paymentsTemplates = function paymentsTemplates(payments) {
      return Object(lit_html__WEBPACK_IMPORTED_MODULE_1__["html"])(_templateObject13 || (_templateObject13 = _taggedTemplateLiteral(["", ""])), payments.map(function (payment, index) {
        var isFirst = index === 0;
        var selected = _this4.selected ? _this4.selected.code : null;
        return Object(_templates__WEBPACK_IMPORTED_MODULE_3__["paymentTemplate"])(payment, isFirst, selected);
      }));
    };

    Object(lit_html__WEBPACK_IMPORTED_MODULE_1__["render"])(paymentsTemplates(availiblePayments), $('#totalDelivery-2')[0]);
    $('#totalDelivery-2').show();

    $(ELEM_NEXT_STEP).show();

    if (!_this4.init) {
		$.getJSON('/widget/api/payments.json', function (methods) {
			$.each(methods, function (method, val) {
				if ($('input[name="payment"][value="' + method + '"]').length) {
					let label = $('input[name="payment"][value="' + method + '"]').parent('label');

					$('<a class="uk-text-warning" uk-toggle="" href="#modal-' + method + '">Подробнее</a>').insertAfter(label);
					$('<div uk-modal="" class="uk-modal" id="modal-' + method + '"><div class="uk-modal-dialog uk-modal-body"><h3 class="uk-modal-title">' + val.title + '</h3>' + val.desc + '<p class="uk-text-right"><button class="uk-button uk-button-default uk-modal-close" type="button">Закрыть</button> <button type="button" class="uk-button uk-button-default uk-modal-close btnSelectPeyment" data-peyment="' + method + '">Выбрать</button></p></div></div>').insertAfter(label);
				}
			});
		});
	}
  ...
  _defineProperty(this, "selectPayment", function () {
    var code = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : null;

    if (code === null) {
      _this4.selected = null;

      _this4.changePaymentEvent();

      return;
    }

    _this4.selected = _this4.payments.find(function (item) {
  ...
  var localData = localStorage.getItem('payment') ? JSON.parse(localStorage.getItem('payment')) : null;
...
$(function () {
	if (window.location.href.indexOf('order') > -1) {
		$('.rf-form').hide();
		$('.lds-wrapper, .lds-ring').show();
		$('#shopwidget').load(''.concat(document.location.origin, '/widget/order.html'), function() {
			$('.lds-wrapper, .lds-ring').hide();
		});
	} else {
		var localShopData = {
		...
		var SHOP = new Shop(localShopData);
		var STEPS = SHOP.STEPS;
		var DELEVERY = SHOP.DELEVERY;
		var PAYMENT = SHOP.PAYMENT;
  ...
  document.addEventListener('shop:changeStep', function () {
    if (STEPS.step === 2) {
      if (!DELEVERY.init) {
        DELEVERY.initDelivery();
      }
  ...
  document.addEventListener('shop:changeDelivery', function () {
    $(document).find('.totalPrice').text(Math.round(parseFloat(SHOP.totalPrice) - parseFloat(SHOP.availibleSales.total) + parseFloat(DELEVERY.cost) + parseFloat(SHOP.availibleSales.total_install)));
  });
  document.addEventListener('shop:changeShop', function () {
    $(document).find('.totalPrice').text(Math.round(parseFloat(SHOP.totalPrice) - parseFloat(SHOP.availibleSales.total) + parseFloat(DELEVERY.cost) + parseFloat(SHOP.availibleSales.total_install)));
  }); // Общая сумма

  $(document).find('.totalPrice').text(Math.round(parseFloat(SHOP.totalPrice) + parseFloat(DELEVERY.cost))); // получение товаров
	$('.lds-wrapper, .lds-ring').show();
  jquery__WEBPACK_IMPORTED_MODULE_0___default.a.get("/widget/api/products.php?ids=".concat($('#shopwidget').data('products')), function (response) {
	  $('.lds-wrapper, .lds-ring').hide();
  	if (!response.error) {
      SHOP.initProducts(response.data);
		$('#products .el-item > div, #products .el-item > h3').wrapAll('<div class="uk-child-width-expand uk-flex-middle uk-grid"></div>');
		$('#products .el-item > div > h3, #products .el-item > div > div:last-child').wrapAll('<div class="uk-margin-remove-first-child"></div>');
		$('#products .el-item > div > div:first-child').addClass('uk-width-1-3 uk-first-column').attr('uk-grid', '');
		setTimeout(function(){$('#products .el-item > div > div:first-child').removeClass('uk-grid uk-grid-stack')}, 500);
    } else {
      alert(response.message);
    }
  ...
  $(document).on('click', '.products a.add', function () {
    ...
    var count = +$(this).parents('.el-item').find('input.count').val();
    $(this).removeClass('add').addClass('remove').text('Удалить');
    SHOP.addProduct(id, count, offerID);
  });
  $(document).on('click', 'div[id^=modal-] .add', function () {
    var id = +$(this).data('product');
    var offerID = +$(this).data('offerid');
    var count = 1;
    $(this).removeClass('add').addClass('remove').text('Удалить из заказа');
    $(document).find(".products a[data-product=".concat(id, "].add")).removeClass('add').addClass('remove').text('Удалить из заказа');
    SHOP.addProduct(id, count, offerID);
  ...
  $(document).on('click', 'div[id^=modal-] .remove', function () {
    var id = +$(this).data('product');
    $(this).removeClass('remove').addClass('add').text('Добавить к заказу');
    $(document).find(".products a[data-product=".concat(id, "].remove")).removeClass('remove').addClass('add').text('В корзину');
    SHOP.removeProduct(id);
  }); // изменение кол-ва товара

	$(document).on('click', '.btnSelectPeyment', function () {
		let val = $(this).data('peyment');
		$('input[name="payment"][value="' + val + '"]').prop('checked', true).trigger('click');
	});

  $(document).on('click', '.products a.changeCount', function () {
    var id = +$(this).parents('.el-item').data('id');
    var count = +$(this).parents('.el-item').find('input.count').val();
    ...
    if ($(this).hasClass('plus')) {
      ...
      if (!in_cart) {
		  var offerID = +$(this).parents('.el-item').find('.btnCart').data('offerid');

		  $(this).parents('.el-item').find('.btnCart').removeClass('add').addClass('remove').text('Удалить');
		  SHOP.addProduct(id, nextCount, offerID);
	  } else {
		  SHOP.changeCount(id, nextCount);
	  }
    } else if ($(this).hasClass('minus')) {
      var _nextCount = count - 1;
      ...
      if (_nextCount === 0) {
        ...
        $(this).parents('.el-item').find('input.count').val(_nextCount);
        SHOP.changeCount(id, _nextCount);
      }
    }
  }); // следующий шаг
	
	$(document).on('keypress keyup blur', 'input.count', function(event) {
	...
	UIkit.util.on(document, 'beforeshow', function($e){
		...
		if (typeof $e.detail[0].$options.name !== 'undefined' && $e.detail[0].$options.name == 'modal'){
			$(target).addClass('uk-flex-top uk-flex uk-open');
			$('.uk-modal-dialog', target).addClass('uk-margin-auto-vertical');
		}
	});

  $(document).on('click', ELEM_NEXT_STEP, function () {
  	if ($(this).attr('id') == 'step1_btn') {
  		if (SHOP.cart.length == 0) {
			UIkit.modal.alert('Выберите хоть один товар', {center: true});

			return;
		} else {
			if ($('#products_attr .btnCart.remove').length == 0) {
				UIkit.modal('<div uk-modal class="uk-modal uk-flex-top uk-flex uk-open" id="modal-1"><div class="uk-modal-dialog uk-modal-body uk-margin-auto-vertical"><p>Продолжить без выбора доп. аксессуаров?</p><p class="uk-text-right"><button type="button" class="uk-button uk-button-default uk-modal-close scrollAttr">Нет</button> <button class="uk-button uk-button-default uk-modal-close nextStep" type="button">Да</button></p></div></div>').show();
	  ...
	  if (STEPS.step == 1) {
		  ...
		  if ($('#selectTypeDelivery').find('.uk-flex-center').html() != '') {
			  DELEVERY.service = null;
			  DELEVERY.provider = null;
			  DELEVERY.deliveryType = null;
			  DELEVERY.drawDeliveries();
			  DELEVERY.drawTotalDelivery();
			  DELEVERY.changeDeliveryEvent();
			  localStorage.setItem('payment', JSON.stringify({
				  selected: null
			  }));
			  PAYMENT.changePaymentEvent();
			  PAYMENT.selectPayment();

			  $(document).find('.selectDelivery').removeClass('selected');
			  $('#mapPvzshow, #selectCurier, #totalDelivery, #selectMapPoint').hide();
			  $(document).find('#addressCurier').val('');
			  $(document).find('#houseCurier').val('');
			  $(ELEM_NEXT_STEP).hide();
		  }
	  }

  	if (STEPS.step == 2) {
		if (DELEVERY.service === 'toDoor' && $.trim($('#houseCurier').val()) == '') {
			UIkit.modal.alert('Введите улицу, дом, квартиру');
		} else {
			if ($('input[name="payment"]').is(':checked')) {
				STEPS.next();
			} else {
				UIkit.modal.alert('Выберите оплату');
			}
		}
	} else {
		STEPS.next();
  ...
  $(document).on('click', ELEM_PREV_STEP, function () {
  	$('#step1_btn').show();

  	if ($(this).attr('title') == 'Изменить заказ') {
		STEPS.step = 1;
  		STEPS.setStep(1);
	...
	$(document).on('click', '.scrollAttr', function () {
		let top_p = 100;
		if( /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent) ) {
			top_p = 0;
		}
  ...
  $(document).on('focus', '#country', function (event) {
	  event.preventDefault();
	  if (citiesListtimer) {
		  clearTimeout(citiesListtimer);
	  }
  ...
  $(document).on('keyup', '#city', function () {
	...
	$(document).on('blur', '#city', function () {
		setTimeout(function(){$('#citiesList').hide()}, 1000);
	});

	$(document).on('blur', '#country', function () {
		citiesListtimer = setTimeout(function(){$('#contriesList').hide()}, 1000);
	});


	$(document).on('blur', '#addressCurier', function () {
		setTimeout(function(){$('#addressList').hide()}, 1000);

		//if ($.trim($('#houseCurier').val()) == '') {
			//$(this).addClass('errorInp');
			//UIkit.modal.alert('Введите улицу, дом, квартиру');
		//} else {
  ...
  $(document).on('change', '#city', function () {
	  $('#mapPvzshow, #totalDelivery, #totalDelivery, #selectMapPoint, #selectCurier').hide();
	  $(document).find('.selectDelivery').removeClass('selected');
  }); // выбор города
  ...
  $(document).on('click', '.selectDelivery', function () {
    ...
    PAYMENT.selectPayment();
    $(document).find('.totalPrice').text(parseFloat(SHOP.totalPrice) + parseFloat(DELEVERY.cost));
    $(ELEM_NEXT_STEP).hide();
  }); // выбор пвз - TOTAL вывод

	$(document).on('click', '#mapPvzshow', function () {
		$(this).hide();
		$('#selectMapPoint').show();
	});
  ...
  $(document).on('click', '.selectPvz', function () {
  	var obj = $(this);
	  UIkit.modal.confirm('Подтверждаете этот пункт выдачи?', {
		  labels: {
			  cancel: 'Нет',
			  ok: 'Да'
		  }}).then(function() {
		  ...
		  DELEVERY.selectPvz(+obj.data('id'));
		  PAYMENT.initPayments(DELEVERY.city.data.country_iso_code, DELEVERY.service, DELEVERY.address);
		  $(ELEM_NEXT_STEP).show();
	...
	var initPaymentBool = false;
	$(document).on('change', '#addressCurier', function () {
		/*if (!initPaymentBool) {//icom
			PAYMENT.initPayments(DELEVERY.city.data.country_iso_code, DELEVERY.service, DELEVERY.address);
			initPaymentBool = true;
		}*/
  ...
  $(document).on('keyup', '#addressCurier', function () {
    /*if ($(this).val().length === 1) {
      $(this).val("".concat(DELEVERY.city.value, ", ").concat($(this).val()));
    }*/
  ...
  $(document).on('submit', '#step-3 form', function (e) {
    ...
    jquery__WEBPACK_IMPORTED_MODULE_0___default.a.post("/widget/api/order.php", {
      type: 'create',
      order: {
        products: SHOP.cart,
        discount: SHOP.availibleSales.total,
        cartCost: SHOP.totalPrice,
		margin: SHOP.availibleSales.total_install,
        delivery: {
          cost: DELEVERY.cost,
      ...
      if (!response.error) {
		  $.getJSON('/widget/api/config.json', function (conf) {
			  ...
			  $.each(metrika.preorder, function (goal, ids) {
				  let pregoal = getCookie('pregoal' + goal);
		  ...
		  window.history.replaceState(null, null, '?order=' + response.data.number);
		  $('#shopwidget').load(''.concat(document.location.origin, '/widget/order.html'), function() {
			  $('.lds-wrapper, .lds-ring').hide();
		  });
      } else {
...
/***/ "./src/templates/cart-product.js":
/*!***************************************!*\
  !*** ./src/templates/cart-product.js ***!
  \***************************************/
/*! exports provided: cartProductTemplate */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "cartProductTemplate", function() { return cartProductTemplate; });
...
var cartProductTemplate = function cartProductTemplate(data, index) {
  return Object(lit_html__WEBPACK_IMPORTED_MODULE_0__["html"])(_templateObject || (_templateObject = _taggedTemplateLiteral(["\n<div class=\"uk-grid-margin ", "\">\n    <div\n        class=\"el-item uk-panel uk-margin-remove-first-child\">\n        <div class=\"uk-child-width-expand uk-grid\"\n            uk-grid=\"\">\n            <div\n                class=\"uk-width-auto@s uk-first-column\">\n                <img src=\"", "\"\n                    width=\"100\"\n                    height=\"100\"\n                    class=\"el-image\"\n                    alt=\"\"\n                    decode=\"async\">\n            </div>\n            <div\n                class=\"uk-margin-remove-first-child\">\n                <div class=\"uk-child-width-expand uk-grid\"\n                    uk-grid=\"\">\n                    <div class=\"uk-width-1-2 uk-margin-remove-first-child uk-first-column\">\n                        <h3 class=\"el-title uk-margin-top uk-margin-remove-bottom\">", "</h3>\n                        <div class=\"el-meta uk-text-meta uk-margin-top\">", " \u0448\u0442.</div>\n                    </div>\n                    <div class=\"uk-margin-remove-first-child\">\n                        <div\n                            class=\"el-content uk-panel uk-margin-top\">", " \u0440\u0443\u0431.</div>\n                    </div>\n                </div>\n            </div>\n        </div>\n    </div>\n</div>\n"])), index === 0 ? 'uk-first-column' : '', data.image, data.name, data.count, data.price * data.count);
};

/***/ }),

/***/ "./src/templates/delivery.js":
/*!***********************************!*\
  !*** ./src/templates/delivery.js ***!
  \***********************************/
/*! exports provided: deliveryTemplate, totalDeliveryTemplate */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "deliveryTemplate", function() { return deliveryTemplate; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "totalDeliveryTemplate", function() { return totalDeliveryTemplate; });
/* harmony import */ var lit_html__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! lit-html */ "./node_modules/lit-html/lit-html.js");
...
function num_word(value) {
  var words = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : ['день', 'дня', 'дней'];
  value = Math.abs(value) % 100;
  var num = value % 10;
  if (value > 10 && value < 20) return words[2];
  if (num > 1 && num < 5) return words[1];
...
var deliveryTemplate = function deliveryTemplate(data) {
  ...
  return Object(lit_html__WEBPACK_IMPORTED_MODULE_0__["html"])(_templateObject || (_templateObject = _taggedTemplateLiteral(["\n<div>\n    <div class=\"el-item uk-card uk-card-secondary uk-card-body uk-margin-remove-first-child\">\n        <canvas class=\"uk-background-cover el-image\"\n            height=\"50\"\n            style=\"background-image: url('", "');\"></canvas>\n        <h3 class=\"el-title uk-h3 uk-margin-top uk-margin-remove-bottom\">", "</h3>\n        <div class=\"el-content uk-panel uk-margin-top\">\n            <div class=\"uk-child-width-1-2 uk-grid\" uk-grid=\"\">\n                ", "\n                ", "\n            </div>\n        </div>\n    </div>\n</div>    \n"])), data.image, data.name, data.deliveryToPoint ? Object(lit_html__WEBPACK_IMPORTED_MODULE_0__["html"])(_templateObject2 || (_templateObject2 = _taggedTemplateLiteral(["\n                <div class=\"uk-first-column\">\n                    <p>\u0421\u0430\u043C\u043E\u0432\u044B\u0432\u043E\u0437:\n                        <br>", " \u0440\u0443\u0431., ", " ", "\n                        <br>\u0411\u0435\u0437 \u0443\u0447\u0451\u0442\u0430 \u0434\u043D\u044F \u043E\u0442\u043F\u0440\u0430\u0432\u043A\u0438</p>\n                    <a type=\"button\"\n                        data-service=\"toPoint\" \n                        data-index=\"", "\"\n                        data-provider=\"", "\"\n                        data-cost=\"", "\"\n                        class=\"el-content uk-button uk-margin-top uk-button-default uk-align-center selectDelivery ", "\"\n                    >\n                        \u0412\u044B\u0431\u0440\u0430\u0442\u044C\n                    </a>\n                </div>\n                "])), data.deliveryToPoint.deliveryCost, data.deliveryToPoint.daysMax, num_word(data.deliveryToPoint.daysMax), data.index, data.providerKey, data.deliveryToPoint.deliveryCost, provider && provider === data.providerKey && service === 'toPoint' ? 'selected' : '') : lit_html__WEBPACK_IMPORTED_MODULE_0__["nothing"], data.deliveryToDoor ? Object(lit_html__WEBPACK_IMPORTED_MODULE_0__["html"])(_templateObject3 || (_templateObject3 = _taggedTemplateLiteral(["\n                <div>\n                    <p>\u041A\u0443\u0440\u044C\u0435\u0440:\n                        <br>", " \u0440\u0443\u0431., ", " ", "\n                        <br>\u0411\u0435\u0437 \u0443\u0447\u0451\u0442\u0430 \u0434\u043D\u044F \u043E\u0442\u043F\u0440\u0430\u0432\u043A\u0438</p>\n                    <a type=\"button\" data-service=\"toDoor\" data-index=\"", "\" data-provider=\"", "\" data-cost=\"", "\"\n                        class=\"el-content uk-button uk-margin-top uk-button-default uk-align-center selectDelivery ", "\">\n                        \u0412\u044B\u0431\u0440\u0430\u0442\u044C\n                    </a>\n                </div>\n                "])), data.deliveryToDoor.deliveryCost, data.deliveryToDoor.daysMax, num_word(data.deliveryToDoor.daysMax), data.index, data.providerKey, data.deliveryToDoor.deliveryCost, provider && provider === data.providerKey && service === 'toDoor' ? 'selected' : '') : lit_html__WEBPACK_IMPORTED_MODULE_0__["nothing"]);
};
var totalDeliveryTemplate = function totalDeliveryTemplate(data) {
  return Object(lit_html__WEBPACK_IMPORTED_MODULE_0__["html"])(_templateObject4 || (_templateObject4 = _taggedTemplateLiteral(["\n<div class=\"el-item uk-panel uk-margin-remove-first-child\">\n    <h3 class=\"el-title uk-margin-top uk-margin-remove-bottom\">\u0414\u043E\u0441\u0442\u0430\u0432\u043A\u0430: </h3>\n    <div class=\"el-content uk-panel uk-margin-top\">\n        <p>", " ", ": ", "\n            <br>\u0421\u0442\u043E\u0438\u043C\u043E\u0441\u0442\u044C \u0434\u043E\u0441\u0442\u0430\u0432\u043A\u0438: ", " \u0440\u0443\u0431\u043B\u0435\u0439\n            <br> Скидка: <span id=\"order_sales_total\"></span> \u0440\u0443\u0431\u043B\u0435\u0439\n<br> \u0418\u0442\u043E\u0433\u043E \u0441\u0442\u043E\u0438\u043C\u043E\u0441\u0442\u044C \u0442\u043E\u0432\u0430\u0440\u043E\u0432 \u0441 \u0434\u043E\u0441\u0442\u0430\u0432\u043A\u043E\u0439: <span class=\"uk-h4 uk-text-warning\"><span class=\"totalPrice\"></span> \u0440\u0443\u0431\u043B\u0435\u0439</span></p>\n    </div>\n</div>\n"])), data.service, data.provider, data.address, data.cost);
};
...
/*!********************************!*\
  !*** ./src/templates/index.js ***!
  \********************************/
/*! exports provided: productTemplate, deliveryTemplate, totalDeliveryTemplate, paymentTemplate, productTotalTemplate, cartProductTemplate */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony import */ var _product__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./product */ "./src/templates/product.js");
/* harmony reexport (safe) */ __webpack_require__.d(__webpack_exports__, "productTemplate", function() { return _product__WEBPACK_IMPORTED_MODULE_0__["productTemplate"]; });

/* harmony reexport (safe) */ __webpack_require__.d(__webpack_exports__, "productTotalTemplate", function() { return _product__WEBPACK_IMPORTED_MODULE_0__["productTotalTemplate"]; });

/* harmony import */ var _delivery__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./delivery */ "./src/templates/delivery.js");
/* harmony reexport (safe) */ __webpack_require__.d(__webpack_exports__, "deliveryTemplate", function() { return _delivery__WEBPACK_IMPORTED_MODULE_1__["deliveryTemplate"]; });

/* harmony reexport (safe) */ __webpack_require__.d(__webpack_exports__, "totalDeliveryTemplate", function() { return _delivery__WEBPACK_IMPORTED_MODULE_1__["totalDeliveryTemplate"]; });

/* harmony import */ var _payments__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ./payments */ "./src/templates/payments.js");
/* harmony reexport (safe) */ __webpack_require__.d(__webpack_exports__, "paymentTemplate", function() { return _payments__WEBPACK_IMPORTED_MODULE_2__["paymentTemplate"]; });

/* harmony import */ var _cart_product__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ./cart-product */ "./src/templates/cart-product.js");
/* harmony reexport (safe) */ __webpack_require__.d(__webpack_exports__, "cartProductTemplate", function() { return _cart_product__WEBPACK_IMPORTED_MODULE_3__["cartProductTemplate"]; });
...
/***/ "./src/templates/payments.js":
/*!***********************************!*\
  !*** ./src/templates/payments.js ***!
  \***********************************/
/*! exports provided: paymentTemplate */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "paymentTemplate", function() { return paymentTemplate; });
/* harmony import */ var lit_html__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! lit-html */ "./node_modules/lit-html/lit-html.js");
var _templateObject;
...
var paymentTemplate = function paymentTemplate(data) {
  var first = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
  var selected = arguments.length > 2 ? arguments[2] : undefined;
  return Object(lit_html__WEBPACK_IMPORTED_MODULE_0__["html"])(_templateObject || (_templateObject = _taggedTemplateLiteral(["\n<div class=\"", "\">\n    <label>\n        <input type=\"radio\" name=\"payment\" value=\"", "\" ?checked=", ">&nbsp;&nbsp;", "\n    </label>\n</div>\n"])), !first ? 'uk-margin-top' : '', data.code, selected && data.code === selected, data.name);
};
...
/*!**********************************!*\
  !*** ./src/templates/product.js ***!
  \**********************************/
/*! exports provided: productTemplate, productTotalTemplate */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "productTemplate", function() { return productTemplate; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "productTotalTemplate", function() { return productTotalTemplate; });
/* harmony import */ var lit_html__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! lit-html */ "./node_modules/lit-html/lit-html.js");
...
var productTemplate = function productTemplate(data, options) {
  return Object(lit_html__WEBPACK_IMPORTED_MODULE_0__["html"])(_templateObject || (_templateObject = _taggedTemplateLiteral(["\n<div>\n    <div\n        class=\"el-item uk-card uk-card-secondary uk-card-body uk-margin-remove-first-child\" data-id=\"", "\" data-offerid=\"", "\">\n        <div uk-lightbox><a href=\"", "\"><canvas class=\"uk-background-cover el-image\" width=\"1661\" height=\"1658\"\n            style=\"background-image: url(&quot;", "#thumbnail=%2C&amp;srcset=1&quot;);\"></canvas></a></div>\n        <h3 class=\"el-title uk-h4 uk-margin-top uk-margin-remove-bottom\">", "</h3>\n        <div class=\"el-content uk-panel uk-margin-top\">\n            <div class=\"uk-h4\">\n                <div><a class=\"uk-text-warning\" href=\"#modal-", "\"\n                        uk-toggle=\"\">\u041F\u043E\u0434\u0440\u043E\u0431\u043D\u0435\u0435</a></div>\n\n                <div class=\"uk-margin-top\"><span\n                        class=\"uk-h5 uk-text-warning \">\u0426\u0435\u043D\u0430:</span> \n                        ", " <span class=\"uk-h5\">", " \u0440.</span>\n                </div>\n            </div>\n            <div class=\" uk-text-center\">\n                <div class=\"uk-inline\">\n                    <a class=\"uk-padding-small uk-icon uk-position-center-right changeCount plus\"\n                        uk-icon=\"icon: plus; ratio: .75\"><svg width=\"15\" height=\"15\"\n                            viewBox=\"0 0 20 20\" xmlns=\"http://www.w3.org/2000/svg\"\n                            data-svg=\"plus\">\n                            <rect x=\"9\" y=\"1\" width=\"1\" height=\"17\"></rect>\n                            <rect x=\"1\" y=\"9\" width=\"17\" height=\"1\"></rect>\n                        </svg></a>\n                    <a class=\"uk-padding-small uk-position-center-left uk-icon changeCount minus\"\n                        uk-icon=\"icon: minus; ratio: .75\"><svg width=\"15\" height=\"15\"\n                            viewBox=\"0 0 20 20\" xmlns=\"http://www.w3.org/2000/svg\"\n                            data-svg=\"minus\">\n                            <rect height=\"1\" width=\"18\" y=\"9\" x=\"1\"></rect>\n                        </svg></a>\n                    <input class=\"uk-input uk-text-center count\" type=\"text\" value=\"", "\">\n                </div>\n            </div>\n            ", "\n            <a\n                type=\"button\"\n                data-product=\"", "\"\n                data-offerid=\"", "\"\n                class=\"el-content uk-button uk-margin-top uk-button-default uk-align-center btnCart ", "\"\n            >\n                ", "\n            </a>\n        </div>\n    </div>\n</div>\n"])), data.id, data.offerID, data.image, data.image, data.name, data.id, data.extra && data.extra.oldPrice ? Object(lit_html__WEBPACK_IMPORTED_MODULE_0__["html"])(_templateObject2 || (_templateObject2 = _taggedTemplateLiteral(["<s class=\"uk-h5\">", "</s>"])), data.extra.oldPrice) : lit_html__WEBPACK_IMPORTED_MODULE_0__["nothing"], data.price, options.count, data.extra ? Object(lit_html__WEBPACK_IMPORTED_MODULE_0__["html"])(_templateObject3 || (_templateObject3 = _taggedTemplateLiteral(["\n                <div id=\"modal-", "\" uk-modal=\"\" class=\"uk-modal\">\n                    <div class=\"uk-modal-dialog uk-modal-body\">\n                        <h3 class=\"uk-modal-title\">", "</h3>\n                        ", "\n                        <p class=\"uk-text-right\">\n                            <button class=\"uk-button uk-button-default uk-modal-close\"\n                                type=\"button\">\u0417\u0430\u043A\u0440\u044B\u0442\u044C</button>\n                            <button class=\"uk-button uk-button-default uk-modal-close btnCart ", "\"\n                                type=\"button\" data-offerid=\"", "\" data-product=\"", "\">", "</button>\n                        </p>\n                    </div>\n                </div>\n            "])), data.id, data.extra.popup.title, Object(lit_html__WEBPACK_IMPORTED_MODULE_0__["html"])(_templateObject4 || (_templateObject4 = _taggedTemplateLiteral(["", ""])), Object(lit_html_directives_unsafe_html__WEBPACK_IMPORTED_MODULE_1__["unsafeHTML"])(data.extra.popup.desc)), options.inCart ? 'remove' : 'add', data.offerID, data.id, options.inCart ? 'Удалить из заказа' : 'Добавить к заказу') : lit_html__WEBPACK_IMPORTED_MODULE_0__["nothing"], data.id, data.offerID, options.inCart ? 'remove' : 'add', options.inCart ? 'Удалить' : 'В корзину');
};
var productTotalTemplate = function productTotalTemplate(count, price, sale) {
  return Object(lit_html__WEBPACK_IMPORTED_MODULE_0__["html"])(_templateObject5 || (_templateObject5 = _taggedTemplateLiteral(["\n<p>\u0412\u044B\u0431\u0440\u0430\u043D\u043E <span class=\"totalProd\">", "</span> ", " \u043D\u0430 \u0441\u0443\u043C\u043C\u0443 <span>", "</span> \u0440\u0443\u0431\u043B\u0435\u0439\n    <br>\u0421\u043A\u0438\u0434\u043A\u0430: <span id=\"sales_total\">", "</span> \u0440\u0443\u0431\u043B\u0435\u0439\n    <br>\u0418\u0442\u043E\u0433\u043E \u0441\u0442\u043E\u0438\u043C\u043E\u0441\u0442\u044C \u0442\u043E\u0432\u0430\u0440\u043E\u0432: <span class=\"uk-h4 uk-text-warning\"><span id=\"total_price\">", "</span>\n        \u0440\u0443\u0431\u043B\u0435\u0439</span>\n</p>\n"])), count, num_word(count), price, sale, price - sale);
};
